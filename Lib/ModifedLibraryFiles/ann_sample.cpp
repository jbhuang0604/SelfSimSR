...
char			SaveFileName[300] = "Result.txt";
...
//			cout << "," << i << "," << nnIdx[i] << "," << dists[i];
			fprintf_s( pf , ",%d,%f" , nnIdx[i] , dists[i] );
		}
		//cout << "\n";
		fprintf_s( pf , "\n" );
		idx++;
...
void getArgs(int argc, char **argv)
{
	...
	if (argc <= 1) {							// no arguments
		...
		<< "    savefilename name of the file result output\n\n"
		<< " Results are sent to the standard output.\n"
		<< "\n"
	...
	while (i < argc) {							// read arguments
		...
		else if (!strcmp(argv[i], "-sa")) {		// -sa option
